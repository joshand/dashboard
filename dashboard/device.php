            ...
            <!-- Custom Tabs -->
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                  ...
                  <?php echo CheckFeatureSupport($arr_devinfo["featurename"],$arr_devinfo["featurename2"],"SUPPORTS_SWITCHING","<li><a href='#tab_2' data-toggle='tab'>Switching</a></li>"); ?>
                  <?php echo CheckFeatureSupport($arr_devinfo["featurename"],$arr_devinfo["featurename2"],"SUPPORTS_ROUTING","<li><a href='#tab_3' data-toggle='tab'>Routing</a></li>"); ?>
                  <?php echo CheckFeatureSupport($arr_devinfo["featurename"],$arr_devinfo["featurename2"],"SUPPORTS_WIRELESS","<li><a href='#tab_4' data-toggle='tab'>Wireless</a></li>"); ?>
                  <?php echo CheckFeatureSupport($arr_devinfo["featurename"],$arr_devinfo["featurename2"],"SUPPORTS_VOICE","<li><a href='#tab_5' data-toggle='tab'>Collaboration</a></li>"); ?>
                  <?php echo CheckFeatureSupport($arr_devinfo["featurename"],$arr_devinfo["featurename2"],"SUPPORTS_SERVER","<li><a href='#tab_6' data-toggle='tab'>Compute</a></li>"); ?>
                  <li<?php echo $tab7act; ?>><a href="#tab_7" data-toggle="tab">Collector</a></li>
                </ul>
		          ...
		          <!--<span class="label label-primary">Label</span>-->
                      ...
                      <div class="box-body">
                        <dl class="dl-horizontal">
                          <dt>Management IP:</dt>
                          <dd><?php echo $arr_devinfo["lanip"]; ?></dd>
<?php
/*
                          <dt>MAC Address:</dt>
...
?>
                          ...
                          <dd>
			      <?php
				echo $arr_devinfo["serialnum"] . " <font color='gray'>(";
				if($arr_devinfo["moddesc"]=="") {
					echo $arr_devinfo["moddesc2"];
				} else {
					echo $arr_devinfo["moddesc"];
				}
				echo ")</font>";
			      ?>
			  </dd>
...
?>
      ...
      <div class="control-sidebar-bg"></div>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" placeholder="IP Address" value="<?php echo $arr_devinfo["lanip"]; ?>" name="mgmt_ip_address" id="mgmt_ip_address">
                    </div>
                    <div class="form-group">
...
?>
    	...
    	$("#savebutton1, #savebutton2").click(function(){
		  ...
		  var dpid = $("#dev_pid").val();
		  var dser = $("#dev_series").val();
	 	  var etype = 1;
	      console.log( id + "-" + ip);
	      $.ajax({
            datatype : "json",
	...
	//Pre-populate device selection dropdowns
        //var devpid = $(dev_pid).val();
	var devpid = "<?php echo $arr_devinfo["refDeviceModel"]; ?>";
        $.ajax({
        	type: "POST",
                data: {devpid: devpid},
                url: 'sp_devset.php',
...
function GetDeviceInfo($dbt,$dbs,$dbn,$dbu,$dbp,$did) {
	$str_pkid = "";
	//$strSQL = "SELECT elements.*,elementports.description,typedevicemodel.description AS moddesc,typedeviceseries.name AS moddesc2,devicetags.tag,GROUP_CONCAT(typefeature.Name) AS featurename FROM elements LEFT JOIN elementports ON (elements.fkdgport = elementports.pkid) LEFT JOIN typedevicemodel ON (elements.refDeviceModel = typedevicemodel.ID) LEFT JOIN devicetags ON (elements.pkid = devicetags.fkelement) LEFT JOIN typedeviceseries ON ((elements.refDeviceModel * -1) = typedeviceseries.id) LEFT JOIN typedevicefamily ON (typedeviceseries.fkfamily = typedevicefamily.id) LEFT JOIN familyfeaturemap ON (typedevicefamily.id = familyfeaturemap.fkdevicefamily) LEFT JOIN typefeature ON (familyfeaturemap.fkfeature = typefeature.id) WHERE (elements.pkid = '" . CleanSQL($did) . "')";
	$strSQL = "SELECT elements.*,elementports.description,typedevicemodel.description AS moddesc,typedeviceseries.name AS moddesc2,devicetags.tag,GROUP_CONCAT(tf1.Name) AS featurename,GROUP_CONCAT(tf2.Name) AS featurename2 FROM elements LEFT JOIN elementports ON (elements.fkdgport = elementports.pkid) LEFT JOIN typedevicemodel ON (elements.refDeviceModel = typedevicemodel.ID) LEFT JOIN typedeviceseries AS tds1 ON (typedevicemodel.fkseries = tds1.id) LEFT JOIN typedevicefamily AS tdf1 ON (tds1.fkfamily = tdf1.id) LEFT JOIN familyfeaturemap AS ffm1 ON (tdf1.id = ffm1.fkdevicefamily) LEFT JOIN typefeature AS tf1 ON (ffm1.fkfeature = tf1.id) LEFT JOIN devicetags ON (elements.pkid = devicetags.fkelement) LEFT JOIN typedeviceseries ON ((elements.refDeviceModel * -1) = typedeviceseries.id) LEFT JOIN typedevicefamily AS tdf2 ON (typedeviceseries.id = tdf2.id) LEFT JOIN familyfeaturemap AS ffm2 ON (tdf2.id = ffm2.fkdevicefamily) LEFT JOIN typefeature AS tf2 ON (ffm2.fkfeature = tf2.id) WHERE (elements.pkid = '" . CleanSQL($did) . "')";
	//echo "--$strSQL--";
...
function CheckFeatureSupport($fl1,$fl2,$fname,$output) {
	$ret = "";
	if(strpos($fl1,$fname)===false) {
		if(strpos($fl2,$fname)===false) {
			$ret = "";
		} else {
			$ret = $output;
		}
